...
if __name__ == '__main__':
    WORK_DIR = sys.path[0]
    UTILS_DIR = "/".join(WORK_DIR.split('/')[:-1]) + '/utils'
else:
    WORK_DIR = os.path.join(sys.path[0], 'src/dataset/audio2landmark')
    UTILS_DIR = os.path.join(sys.path[0], 'src/dataset/utils')
# print(WORK_DIR, UTILS_DIR)
STD_FACE_LANDMARK_FILE_DIR = os.path.join(UTILS_DIR, 'STD_FACE_LANDMARKS.txt')
...
class Audio2landmark_Dataset(data.Dataset):
    ...
    def __init__(self, dump_dir, dump_name, num_window_frames, num_window_step, status):
        ...
        au_mean_std = np.loadtxt(os.path.join(UTILS_DIR, 'MEAN_STD_AUTOVC_RETRAIN_MEL_AU.txt'))
...
def norm_output_fls_rot(fl_data_i, anchor_t_shape=None):
    ...
    if(anchor_t_shape is None):
        anchor_t_shape = np.loadtxt(
            r'{}/ANCHOR_T_SHAPE_{}.txt'.format(UTILS_DIR,len(t_shape_idx)))
...
class Speaker_aware_branch_Dataset(data.Dataset):
    ...
    def __init__(self, dump_dir, dump_name, num_window_frames, num_window_step, status, use_11spk_only=False, noautovc=''):
        ...
        au_mean_std = np.loadtxt(os.path.join(UTILS_DIR,'MEAN_STD_AUTOVC_RETRAIN_MEL_AU.txt')) # np.mean(self.au_data[0][0]), np.std(self.au_data[0][0])
        au_mean, au_std = au_mean_std[0:au_mean_std.shape[0]//2], au_mean_std[au_mean_std.shape[0]//2:]
