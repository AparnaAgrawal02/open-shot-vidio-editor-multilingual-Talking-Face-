...
if __name__ == '__main__':
    WORK_DIR = sys.path[0] # MakeItTalk/src/approaches
    UTILS_DIR = '/'.join(WORK_DIR.split('/')[:-1]) + '/dataset/utils'
    EXAMPLE_DIR = '/'.join(WORK_DIR.split('/')[:-2]) + '/examples'
else:
    WORK_DIR = sys.path[0] # MakeItTalk
    UTILS_DIR = os.path.join(WORK_DIR,'src/dataset/utils')
...
class Audio2landmark_model():
    ...
    def __init__(self, opt_parser, jpg_shape=None):
        ...
        self.std_face_id = np.loadtxt(os.path.join(UTILS_DIR, 'STD_FACE_LANDMARKS.txt'))
        ...
        self.eval_data = Audio2landmark_Dataset(dump_dir=os.path.join(EXAMPLE_DIR,'dump'),
                                                dump_name='random',
                                                status='val',
        ...
        self.anchor_t_shape = np.loadtxt(os.path.join(UTILS_DIR, 'STD_FACE_LANDMARKS.txt'))
        self.anchor_t_shape = self.anchor_t_shape[self.t_shape_idx, :]

        with open(os.path.join(EXAMPLE_DIR, 'dump', 'emb.pickle'), 'rb') as fp:
            ...
            for key in keys: # ['45hn7-LXDX8']: #['sxCbrYjBsGA']:#
                ...
                if(vis_fls):
                    ...
                    Vis(fls=fake_fls_np, filename=video_name.split('\\')[-1].split('/')[-1], fps=62.5,
                        audio_filenam=os.path.join(EXAMPLE_DIR, video_name.split('\\')[-1].split('/')[-1]+'.wav'))
