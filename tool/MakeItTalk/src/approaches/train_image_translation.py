...
class Image_translation_block():
    ...
    def single_test(self, outfile1, outfile2, jpg=None, fls=None, filename=None, prefix='', grey_only=False,img_height=None,img_width=None):
        import time
        st = time.time()
        self.G.eval()
        ...
        writer = cv2.VideoWriter(outfile1, cv2.VideoWriter_fourcc(*'DIVX'), 62.5, (img_width, img_height))
...
#                 frame = g_out[i] * 255.0
                g_out[i] = g_out[i] * 255.0
                res = cv2.resize(g_out[i], dsize=(img_width, img_height))
                writer.write(res.astype(np.uint8))
        ...
        # if(filename is None):
        #     filename = 'v'
        os.system('ffmpeg -loglevel error -y -i {} -i {} -pix_fmt yuv420p -strict -2 {}'.format(outfile1,filename,outfile2))
