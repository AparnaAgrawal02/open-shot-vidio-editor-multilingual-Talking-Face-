...
import sys
BASE_DIR = os.environ['PWD'] # tool directory
EXAMPLE_DIR = os.path.join(BASE_DIR, 'MakeItTalk', 'examples')
UTILS_DIR = os.path.join(BASE_DIR, 'MakeItTalk', 'src', 'dataset', 'utils')
...
class Vis():
    ...
    def __init__(self, fls, filename, audio_filenam=None, fps=100, frames=1000):
        ...
        writer = cv2.VideoWriter(os.path.join(EXAMPLE_DIR, 'tmp.mp4'),
        ...
        if(audio_filenam is not None):
            ...
            os.system('ffmpeg -y -i {} -i {} -strict -2 -shortest {}'.format(
                os.path.join(EXAMPLE_DIR, 'tmp.mp4'),
                audio_filenam,
                os.path.join(EXAMPLE_DIR, '{}_av.mp4'.format(filename))
            ))
        else:
        ...
        os.remove(os.path.join(EXAMPLE_DIR, 'tmp.mp4'))
...
class Vis_old():
    ...
    def __init__(self, run_name, pred_fl_filename, audio_filename, av_name='NAME', fps=100, frames=625,
        ...
        self.std_face = np.loadtxt(r'{}/STD_FACE_LANDMARKS.txt'.format(UTILS_DIR))
