...
def train(rank, epoch, hps, generator, optimizer_g, train_loader, logger, writer=None):
  ...
  for batch_idx, (x, x_lengths, y, y_lengths) in enumerate(train_loader):
    ...
    print("z", z[:1].shape, x[:1].shape, y[:1].shape)
    loss_gs = [l_mle, l_length]
    loss_g = sum(loss_gs)

    # print(loss_gs, loss_g)
    ...
    if rank==0:
      if batch_idx % hps.train.log_interval == 0:
        ...
        print("ygen", y_gen.shape, x[:1].shape)
...
def evaluate(rank, epoch, hps, generator, optimizer_g, val_loader, logger, writer_eval=None):
  if rank == 0:
    global global_step
    generator.eval()
    losses_tot = []
