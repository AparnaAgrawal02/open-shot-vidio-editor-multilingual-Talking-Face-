...
class Export(QDialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if export_path and os.path.exists(export_path):
            ...
            # Default to home dir
            self.txtExportFolder.setText(os.path.join(recommended_path, 'exports'))
    ...
    def accept(self, start_pos=1, end_pos=1, lipsync=False, NAME=''):
        ...
        if start_pos==1 and end_pos ==1 :
            start_pos = self.txtStartFrame.value()
            end_pos = self.txtEndFrame.value()

        self.progressExportVideo.setMinimum(start_pos)
        self.progressExportVideo.setMaximum(end_pos)
        self.progressExportVideo.setValue(start_pos)
        ...
        if export_type == _("Image Sequence"):
            file_name_with_ext = "%s%s" % ( default_filename or self.txtFileName.text().strip(), self.txtImageFormat.text().strip())
            # file_name_with_ext = "%s%s" % (default_filename, self.txtImageFormat.text().strip())
        else:
            file_ext = self.txtVideoFormat.text().strip()
            # file_name_with_ext = default_filename
            file_name_with_ext = default_filename or self.txtFileName.text().strip()
        ...
        if self.s.get("show_finished_window") and self.exporting:
            ...
            if not lipsync:
                self.show()
            return file_name_with_ext, export_file_path
        else:
            # Accept dialog
